(ns mathplot.core
  ...
  (:require [mathplot.parse :refer [string->fn]]
            ...
            [mathplot.shape :refer
             [shape->paint new-fn-plot new-parameter-plot new-polar-plot]]
            [swinghelp.core :refer [sset-class! sset! sget]]))

;; state

(def plot-modes [:explicit :parameter :polar])
...
(defn- parse-parameter-input [e]
  (let [root (to-root e)
    ...
    (if (and xfn yfn)
      (do
        (add-shapes! (assoc (new-parameter-plot xfn yfn )
                            :color (rand-color)
                            :label (format "x=%s, y=%s" xfn-s yfn-s)))
        (update-root root :paint :display))
      (alert "incorrect input"))))

(defn- parse-input-polar [e]
  (let [root (to-root e)
        s (sget root [:polar :text])]
    (if-let[ f (string->fn s)]
      (do (add-shapes! (assoc (new-polar-plot f)
                              :color (rand-color)
                              :label (format "r=%s" s)))
          (update-root root :paint :display))
      (alert "incorrect input"))))
...
(def mode->input-items
  {:explicit
   ...
   :polar
   [(horizontal-panel
     :items [(label :text "r=" :class :text)
             (text :id :polar :class :text
                   :listen [:action parse-input-polar])])]})
